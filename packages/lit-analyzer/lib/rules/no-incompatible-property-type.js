...
const array_util_js_1 = require("../analyze/util/array-util.js");
...
const rule = {
    ...
    visitComponentMember(member, context) {
        ...
        return validateLitPropertyConfig(member.meta.node?.type || member.meta.node?.decorator?.expression || member.node, member.meta, {
...
function prepareSimpleAssignabilityTester(simpleType) {
    ...
    const acceptedTypeKinds = (0, general_util_js_1.lazy)(() => {
...
function validateLitPropertyConfig(node, litConfig, { propName, simplePropType }, context) {
    ...
    if (litConfig.type != null) {
        ...
        if (!isAssignableTo(litConfig.type.kind) && !isAssignableTo("ANY")) {
            ...
            if (acceptedTypeKinds().length >= 1) {
                const potentialKindText = (0, array_util_js_1.joinArray)(acceptedTypeKinds().map(kind => `'${toLitPropertyTypeString(kind)}'`), ", ", "or");
                context.report({
                    location: (0, range_util_js_1.rangeFromNode)(node),
            ...
            // The OBJECT @property type is an escape from this error
                ...
                context.report({
                    location: (0, range_util_js_1.rangeFromNode)(node),
                    message: `@property type '${(0, ts_simple_type_1.typeToString)(litConfig.type)}' is not assignable to the actual type '${(0, ts_simple_type_1.typeToString)(simplePropType)}'`
                });
            }
        }
    }
    // If no type has been specified, suggest what to use as the @property type
        ...
        // Suggest what to use instead if there are multiple accepted @property types for this property
            ...
            const acceptedTypeText = (0, array_util_js_1.joinArray)([
            ...
            context.report({
                location: (0, range_util_js_1.rangeFromNode)(node),
                message: `The built in converter doesn't handle the property type '${(0, ts_simple_type_1.typeToString)(simplePropType)}'.`,
...
//# sourceMappingURL=no-incompatible-property-type.js.map