	...
	test.after.always(() => {
		...
		if (existsSync("./playground")) {
			...
			if (reproCodeNonStrict.length > 0) {
				writeFileSync(
					"./playground/repro-non-strict.ts",
					`// Command: DEBUG= STRICT=false FILE=repro-non-strict.ts npm run playground\n\n${reproCodeNonStrict}`
				);
			}
		}
	});
...
export function testCombinedTypeAssignment(
	...
	reportError: (reproCode: string) => void = () => {}
) {
	...
	visitComparisonsInTestCode(
		testCode,
		compilerOptions,
		({ assignable: expectedResult, nodeA, checker, program, typeA, typeB, typeAString, typeBString, line }) => {
